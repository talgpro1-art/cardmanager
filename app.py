...
# ----------------------------------------------------------------------
# 2. 데이터베이스(세션) 초기화 (구글 시트 역할)
# ----------------------------------------------------------------------
if 'current_usage' not in st.session_state:
    st.session_state.current_usage = {"신한 RPM카드": 0} # 이번 달 일반 사용액
if 'gift_card_usage' not in st.session_state:
    st.session_state.gift_card_usage = {"신한카드": 0}   # 이번 달 상품권 사용액 (카드사 통합)
if 'last_month_tier' not in st.session_state:
    st.session_state.last_month_tier = {"신한 RPM카드": 1} # 전월 달성 구간 (초기값 1구간 세팅)
...
def end_of_month_process():
    """월 마감 처리: 히스토리 저장 및 이번달 실적 바탕으로 다음달 혜택 세팅"""
...
col_b2.success(f"💳 일반 적립처: [last_tier_idx]}%** 적립")

st.markdown("---")

# --- [중단] 이번 달 실적 트래킹 (다음 달 혜택을 위한 목표) ---
st.subheader(f"📊 이번 달 실적 충족 현황 (다음 달 혜택용)")
